...
```{r}
ref <- NHANES %>% filter(AgeDecade==' 20-29') %>% summarize(average=mean(BPSysAve,na.rm=TRUE),standard_devidation=sd(BPSysAve,na.rm = TRUE))
```
...
```{r}
grades <- tibble(names=c("John","Juan","Jean","Yao"),exam_1=c(95,80,90,85),exam_2=c(90,85,85,90),stringAsFactors=FALSE)
class(grades$names)
```
