...
We should see ```a```, ```b```, and ```c```. If you try to recover the value of a variable that is not in your workspace, you receive an error. For example, if you type ```x``` you will receive the following message: ```Error: object 'x' is not found```.
...
Values remain in the workspace until you end your session or erase them with the function ```rm```. But workspaces also can be saved for later use. In fact, when you quit R, the program asks you if you want to save you workspace. If you do save it, the next time you start R, the program will restore the workspace.
...
To solve anoter equation such as $3x^2+2x-1$, we can copy and paste the code above and then redefine the variables and recompute the solution:
```{r}
a <- 3
b <- 2
c <- -1
...
n*(n+1)/2
```
2. Now use the same forumla to compute the sum of the integers from 1 through 1,000.
```{r}
n <- 1000
n*(n+1)/2
```
...
```{r}
#answer is state, abb, region, population, total.
```
