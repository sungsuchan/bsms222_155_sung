...
![image](https://rafalab.github.io/dsbook/R/img/csv-file.png)
...
The following function are availalbe to read-in spreadsheets:
...
The package provide function to read-in Microsoft Excel formats:
...
1. Use the ```read_csv``` fucntion to read each of the files that the following code saves in the ```files``` object:
```{r}
...
```
```{r}
file.path(path,"2010_bigfive_regents.xls") %>% file.copy(.,"2010_bigfive_regents.xls" ) 
read_xls("2010_bigfive_regents.xls")

file.path(path,"carbon_emissions.csv") %>% file.copy(.,"carbon_emissions.csv")
read_csv("carbon_emissions.csv")

file.path(path,"fertility-two-countries-example.csv") %>% file.copy(.,"fertility-two-countries-example.csv")
read_csv("fertility-two-countries-example.csv")

file.path(path,"HRlist2.txt") %>% file.copy(.,"HRlist2.txt")
read_csv("HRlist2.txt")

file.path(path,"life-expectancy-and-fertility-two-countries-example.csv") %>% file.copy(.,"life-expectancy-and-fertility-two-countries-example.csv")
read_csv("life-expectancy-and-fertility-two-countries-example.csv")

read.csv("murders.csv")

file.path(path,"olive.csv") %>% file.copy(.,"olive.csv")
read_csv("olive.csv")

file.path(path,"RD-Mortality-Report_2015-18-180531.pdf") %>% file.copy(.,"RD-Mortality-Report_2015-18-180531.pdf")
read_csv("RD-Mortality-Report_2015-18-180531.pdf")

file.path(path,"ssa-death-probability.csv") %>% file.copy(.,"ssa-death-probability.csv")
read_csv("ssa-death-probability.csv")
```

2. Note that the last one, the ```olive``` file, gives us a warning. This is becuase the first line of the file is missing the header for the first column.
...
The ```read_csv``` file can read these files directory:
```{r}
dat <- read_csv(url)
```
