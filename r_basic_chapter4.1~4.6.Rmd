...
```{r}
select(murders,state,abb) %>% print()
```
...
```{r}
m <- filter(murders, region %in% c("Northeast","West") & rate <= 1)

my_states <- select(m,state,rate,rank)
my_states
```
...
<font size="4"><center>original data -> select -> filter</center></font>
