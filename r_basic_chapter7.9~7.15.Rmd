...
Here we want to add a line that represents the average murder rate for the entire country. Once we determine the per million rate to be $r$, this is defined by the formula: $y=rx$, with $y$ and $x$ our axes: total murders and population in millions, respectively. In the log-scale this line turns into: $log(y)=log(r)+log(x)$. So in our plot it's a line with slope 1 and intercept $log(r)$. To compute this value, we use our **dplyr** skills:
```{r}
r <- murders %>% 
...
murders %>% ggplot(aes(population/10^6, total, label=abb)) + geom_abline(intercept=log10(r), lty=2, color="darkgrey") + geom_point(aes(col=region),size=3) +
  geom_text_repel() +
  scale_x_log10() +
  scale_y_log10() +
  xlab("Populations in millions (log scale)") +
...
a) ```state``` and ```abb```.
a) ```total_murders``` and ```population_size```.
a) ```total``` and ```population```.
a) ```murders``` and ```size```.
...
a) We need to let ```geom_label``` know what character to use in the plot.
a) The ```geom_label``` geometry does not require x-axis and y-axis values.
a) ```geom_label``` is not a ggplot2 command.
```{r}
#The answer is 'b'.
```
...
a) Because each label needs a different color we map the colors through ```aes```.
a) Use the ```color``` argument in ```ggplot```.
a) Because we want all colors to be bule, we do not need to map colors, just use the color argument in ```geom_label```
```{r}
#The answer is 'd'
```
...
a) Because each label needs a different color we map the colors through the color argument of ```aes```.
a) Use the ```color``` argument in ```ggplot```.
a) Because we want all colors to be bule, we do not need to map colors, just use the color argument in ```geom_label```.
```{r}
#The answer is 'b'
...
```{r}
murders %>% ggplot(aes(population,total)) + geom_label(aes(population,total,label=abb,color=region))
```
