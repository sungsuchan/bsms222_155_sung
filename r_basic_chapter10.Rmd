...
```{r}
data(murders)
murders %>% mutate(murder_rate=total/population*100000) %>% mutate(state=reorder(state,murder_rate)) %>% ggplot(aes(state, murder_rate)) + geom_bar(stat="identity") + coord_flip() + 
  theme(axis.text.y = element_text(size=6)) + xlab("")
```
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/boxplot-with-points-with-jitter-1.png)
```{r}
heights %>%
  ggplot(aes(sex, height)) +
  geom_boxplot(coef=3) + 
...
dat %>%
  mutate(location = ifelse(year == 2010, 1, 2), 
         location = ifelse(year == 2015 &
                             country %in% c("United kingdom", "Portugal"), location+0.22, location), hjust=ifelse(year==2010,1,0)) %>% 
  mutate(year=as.factor(year)) %>% ggplot(aes(year, life_expectancy, group=country)) + geom_line(aes(color=country), show.legend = F) + geom_text(aes(x = location, label=country, hjust=hjust), show.legend = F) + xlab("") + ylab("Life Expectancy")
```
...
library(ggrepel)
dat %>% 
  mutate(year=paste0("life_expectancy_", year)) %>% select(country, year, life_expectancy) %>% 
  ...
  ggplot(aes(average, difference, label=country)) + geom_point() + geom_text_repel() + geom_abline(lty=2) + xlab("Average of 2010 and 2015") + ylab("Difference between 2015 and 2010")
```
...
## **10.9 Encoding a third varialbe**
...
a) When **ggplot2** is not available.
a) When I am in a bakery.
a) Never. Barplots and tables are always better.
...
a) The axis does not start at 0. Judging by the length, it appears Trump received 3 times as many votes when, in fact, it was about 30% more.
a) The colors should be the same.
a) Percentages should be shown as a pie chart.
```{r}
#The answer is 'b'.
```
...
a) The plot on the right is better because it orders the states alphabetically.
a) The plot on the right is better because alphabetical order has nothing to do with the disease and by ordering according to actual rate, we quickly see the states with most and least rates.
a) Both plots should be a pie chart.
...
library(dslabs)
data("murders")
murders %>% mutate(rate=total/population*100000) %>% group_by(region) %>% summarize(avg=mean(rate)) %>% mutate(region=factor(region)) %>% ggplot(aes(region, avg)) + geom_bar(stat="identity") + ylab("Murder Rate Average")
```
...
a) The graph does not show standard errors.
a) It does not show all the data. We do not see the variability within a region and it's possible that the safest are not in the West.
a) The Northeast has the lowest average.
...
```{r}
murders %>% mutate(region=reorder(region, rate, FUN=median)) %>% ggplot(aes(region, rate)) + geom_boxplot() + geom_point()
```
...
a) There must be an error in the code.
a) The colors confuse us.
a) Scatterplots should not be used to compare two variables when we have access to 3.
```{r}
#The answer is 'a'.
```
...
```{r}
us_contagious_diseases %>% filter(disease=="Smallpox" & weeks_reporting >=10) %>% mutate(rate=count/population*10000*52/weeks_reporting) %>% ggplot(aes(year, state, fill = rate)) + 
...
```{r}
us_contagious_diseases %>% filter(disease=="Smallpox" & weeks_reporting >=10) %>% mutate(rate=count/population*10000*52/weeks_reporting) %>% ggplot(aes(year,rate, group=state)) + geom_line()
```
...
```{r}
us_contagious_diseases %>% filter(weeks_reporting >=10 & state == "California") %>% mutate(rate=count/population*10000*52/weeks_reporting) %>% ggplot(aes(year, rate, color = disease)) + geom_line()
```
...
## **10.14 Case study: vaccines and infectious disease**
...
dat <- us_contagious_diseases %>% 
  filter(!state%in%c("Hawaii","Alaska")&disease == the_disease) %>% mutate(rate=count/population*10000*52/weeks_reporting) %>% mutate(state=reorder(state, rate))
```
...
```{r}
dat %>% ggplot(aes(year, state, fill=rate)) + geom_tile(color ="grey50") + scale_x_continuous(expand=c(0,0)) + scale_fill_gradientn(colors = brewer.pal(9, "Reds"),trans = "sqrt") + 
...
```{r}
avg <- us_contagious_diseases %>% filter(disease == the_disease) %>% group_by(year) %>% summarize(us_rate = sum(count, na.rm=TRUE)/sum(population,na.rm=T)*10000)
```
...
dat %>% 
  ...
  geom_line(mapping=aes(year, us_rate), data=avg, size=1) + scale_y_continuous(trans="sqrt", breaks = c(5, 25,125,300)) + ggtitle("Cases per 10,000 by state") + xlab("") + ylab("") + geom_text(data = data.frame(x=1955, y=50), mapping=aes(x,y,label='US average'), color="black") + 
  geom_vline(xintercept = 1963, col="blue")
```
...
```{r}
dat3 <- us_contagious_diseases %>% filter(disease == "Smallpox") %>% mutate(rate= sum(count)/sum(population)*10000)
dat3%>% 
  filter(!is.na(rate)) %>% 
  ggplot() + 
  geom_line(aes(year, rate, group=state), color = "grey50", show.legend=F, alpha=0.2, size=1) + 
  geom_line(mapping=aes(year, us_rate), data=avg, size=1) + scale_y_continuous(trans="sqrt", breaks = c(5, 25,125,300)) + ggtitle("Smallpox rate") + xlab("") + ylab("") + geom_text(data = data.frame(x=1955, y=50), mapping=aes(x,y,label='US average'), color="black") + 
  geom_vline(xintercept = 1963, col="blue")
```
